...
class TestCausalIntegration(unittest.TestCase):
    def setUp(self):
        ...
        self.strategy.regime_classifier.classify = lambda df, stable_threshold=0.6, crash_threshold=0.3: type('obj', (object,), {
            'regime_id': 0, 'stable_trend_prob': 0.9, 'crash_prob': 0.0, 
            'is_tradeable': True, 'is_crash': False
        })
        ...
        def mock_classify(df, stable_threshold=0.6, crash_threshold=0.3):
            ...
            if df.equals(crash_df.tail(100)): 
                return type('obj', (object,), {
                    'regime_id': 2, 'stable_trend_prob': 0.0, 'crash_prob': 0.9, 
                    'is_tradeable': False, 'is_crash': True
                })
            return type('obj', (object,), {
                'regime_id': 0, 'stable_trend_prob': 0.9, 'crash_prob': 0.0, 
                'is_tradeable': True, 'is_crash': False
    ...
    def test_ete_gate(self):
        ...
        ctx = CausalContext(leader_df=self.leader_df, optimal_lag=5, ete_rising=False, is_valid=True)
        self.strategy.set_leader_context(ctx)
