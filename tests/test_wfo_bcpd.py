...
def run_wfo_bcpd():
    print("Starting Walk-Forward Optimization Backtest (BCPD ENABLED)...")
    ...
    # Initialize Backtester with BCPD Enabled
    backtester = MultiLayerBacktester(
        config={
            ...
            "strategy_config": {
                ...
                # New BCPD Configs
                "use_bcpd": True,         
                "bcpd_hazard": 0.01,      # 1% prior probability of shock
                "bcpd_threshold": 0.3     # Trigger crash logic if prob > 30%
            },
        }
    )
    ...
    while current_idx < len(df):
        ...
        if len(test_df) < 100:
            print(f"  Skipping small window ({len(test_df)} bars)")
            break
    ...
    # === Aggregation ===
    print("\n" + "=" * 50)
    print("WALK-FORWARD RESULTS (BCPD ENABLED)")
    print("=" * 50)
