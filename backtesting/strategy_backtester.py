...
class Trade:
    ...
    exit_reason: Optional[str] = (
        None  # 'signal', 'stop_loss', 'emergency', 'end_of_data'
    )
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "trade_stats": {
                ...
                "avg_trade_duration_hours": self.avg_trade_duration.total_seconds()
                / 3600,
            },
            "capital": {"initial": self.initial_capital, "final": self.final_capital},
        }
...
class MultiLayerBacktester:
    ...
    def run(self, df: pd.DataFrame, train_ratio: float = 0.3) -> BacktestResult:
        ...
        if df.empty or len(df) < 100:
            raise ValueError(
                "Insufficient data for backtesting. Need at least 100 bars."
            )
        ...
        for i in range(len(test_df)):
            ...
            if position is not None and self.use_stop_loss:
                ...
                if stop_triggered:
                    ...
                    if position.side == "long":
                        ...
                        cash += position.size * (2 * position.entry_price - exit_price)
            ...
            if signal == -99 and position is not None:
                exit_price = self._apply_slippage(
                    current_price, "short" if position.side == "long" else "long"
                )
                position.exit_time = timestamp
                position.exit_price = exit_price
            ...
            # Process signals
                ...
                cost = size * entry_price + self._calculate_commission(
                    size * entry_price
                )
                ...
                if position is not None and position.side == "long":
                    ...
                    if position.side == "long":
                        ...
                        # Short: Return collateral + pnl = size * (2*entry - exit)
                        # Derived: size*entry + size*(entry - exit)
                        cash += position.size * (2 * position.entry_price - exit_price)
                ...
                if position is None:
                    ...
                    if cost <= cash:
                        ...
                        position = Trade(
                            ...
                            kama_trend=int(meta.get("kama_trend", 0)),
                            smc_confluence=bool(
                                meta.get("smc_short_confluence", False)
                            ),
                        )
            ...
            if position is not None:
                if position.side == "long":
                    ...
                    position_value = (
                        position.size * position.entry_price + unrealized_pnl
                    )
        ...
        if position is not None:
            ...
            exit_price = self._apply_slippage(
                final_price, "short" if position.side == "long" else "long"
            )
    ...
    def _calculate_results(
        self, trades: List[Trade], equity_df: pd.DataFrame, signals_df: pd.DataFrame
    ) -> BacktestResult:
    ...
    async def run_with_live_data(
        self, symbol: str = "BTC/USDT", timeframe: str = "1h", days: int = 90
    ) -> BacktestResult:
        """
...
def run_quick_backtest(
    ...
    **strategy_kwargs,
    ...
    config = {"initial_capital": initial_capital, "strategy_config": strategy_kwargs}
...
if __name__ == "__main__":
    ...
    result = run_quick_backtest(initial_capital=args.capital, symbol=args.symbol)
